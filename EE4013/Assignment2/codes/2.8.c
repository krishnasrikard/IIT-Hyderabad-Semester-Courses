...
int main(){
	...
	for(int i=0;i<m;i++){
		if (i==0){
			...
			temp1->down = (struct Node*)malloc(sizeof(struct Node));
			temp2 = temp1->down;
			temp1 = temp2;
		}
		for(int j=0;j<n;j++){
		...
		for(int j=0;j<n;j++){
			if (j==0){
				if (temp2->data - temp1->data == 0)
					lambda = (temp4->data - temp1->data)/(pow(10,-6));
				else
					lambda = (temp4->data - temp1->data)/(temp2->data - temp1->data);
			}
			else{
				temp1 = temp1->next;
				temp2 = temp2->next;
				...
				if (temp2->data - temp1->data == 0){
					test = (temp4->data - temp1->data)/(pow(10,-6));
				}
				else{
					test = (temp4->data - temp1->data)/(temp2->data - temp1->data);
				}
