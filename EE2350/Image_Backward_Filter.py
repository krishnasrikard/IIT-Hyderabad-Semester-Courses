...
def Apply_Filter_RGB(img,fs = 2):										# Applying Backward Differencing Method to RGB iamge	
...
def Apply_Filter_Grayscale(img,fs = 2):									## Applying Backward Differencing Method to Grayscale image
...
def Pad_rows_G(img):													# Adding Rows to Gray Scale
...
def Pad_columns_G(img):													# Adding Columns to Gray Scale
	i,j = img.shape[0],img.shape[1]
	z = np.zeros((i,1))
...
def Pad_rows_RGB(img):													# Adding Rows to RGB Scale
...
def Pad_columns_RGB(img):												# Adding Columns to RGB Scale
	i,j,k = img.shape[0],img.shape[1],img.shape[2]
...
img_RGB_edges = PIL.Image.fromarray(img_RGB_edges,'RGB')				# Converting a 3D array to Image
