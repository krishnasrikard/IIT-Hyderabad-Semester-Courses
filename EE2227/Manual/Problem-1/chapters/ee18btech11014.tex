...
The cascade of RC Circuits introduces poles in the circuit. The RC Circuits introduces poles at $f=10^{5} Hz, 10^{6} Hz, 10^{7} Hz$ respectively from left to right. So, the value of $v_{gs_{1}}$ is

\begin{align}
v_{gs_{1}} = \dfrac{v_{s}}{\left(1+j\frac{f}{10^{5}}\right)\left(1+j\frac{f}{10^{6}}\right)\left(1+j\frac{f}{10^{7}}\right)}
\end{align}

The MOSFETs used in the circuit are Self-Biased with $V_{GS} = 1.1V$ for all 3 N-MOSFETs. 3 N-MOSFETs are used to achieve a large gain of $10^{5}$. At 3rd N-MOSFET, Terminals of output are swaped, inorder to compensate for the negetive sign of gain.
